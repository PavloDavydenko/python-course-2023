...
Є список articles_dict, в якому міститься опис статей блогу. Кожен елемент цього списку є словником з наступними ключами: прізвища авторів - ключ 'author', 
...
def find_articles(key, letter_case=False) -> list: 
    ...
    if letter_case:
        ...
        for article in articles_dict:
            if article['title'].find(key.lower()) != (-1) or article['author'].find(key.lower()) != (-1) or article['title'].find(key) != (-1) or article['author'].find(key) != (-1):
                found_articles.append(article)
