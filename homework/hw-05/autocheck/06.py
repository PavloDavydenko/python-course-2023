...
регістру незалежна і весь текст має зводитися до нижнього регістру. Для спрощення, будемо вважати, що в рядку є тільки одне заборонене слово.

Передбачити третій параметр функції space_around, який за замовчуванням дорівнює False. Він відповідатиме за те, що функція шукатиме окреме слово чи ні. 
...
def is_spam_words(text, spam_words, space_around=False) -> bool:
    for word in spam_words:
        if word.lower() in text.lower() and not space_around:
            return True
        elif f' {word}.' in (' ' + text.lower()) or f' {word} ' in (' ' + text.lower()) and space_around:
            return True
